...
export class ContractPrefrencesBuss {
    ...
    async getContract(_id: string): Promise<IContractPrefrences> {
        ...
        if (contractpre === null) throw 'cots doest not exits';
        return contractpre;
    }
    ...
    async updateContract(contractpre: IContractPrefrences): Promise<IContractPrefrences> {
        ...
        if (update_contract === null) throw 'category not updated g' + contractpre._id;
    ...
    async getContractPreferenceList(): Promise<IContractPrefrences[]> {
        let ContractPreferenceList: IContractPrefrences[] = await new MainContract().getContractList();
        return ContractPreferenceList;
    }
}
